...
<div class="container mt-4">
    ...
    {{-- Hapus notifikasi sukses update jumlah --}}
    @if(count($carts) === 0)
                    ...
                    <form action="{{ route('user.cart.update', $cart) }}" method="POST" class="d-inline">
                        ...
                        <div class="d-flex align-items-center" style="gap:8px;">
                            <div class="input-group" style="max-width:120px;">
                                <button type="button" class="btn btn-outline-secondary" onclick="var qty=document.getElementById('qty-{{ $cart->id }}');if(qty.value>1){qty.value--;qty.form.submit();}">-</button>
                                <input type="number" name="quantity" id="qty-{{ $cart->id }}" value="{{ $cart->quantity }}" min="1" max="{{ $cart->product->stock }}" class="form-control text-center" style="width:50px;" onchange="this.form.submit()">
                                <button type="button" class="btn btn-outline-secondary" onclick="var qty=document.getElementById('qty-{{ $cart->id }}');if(qty.value<{{ $cart->product->stock }}){qty.value++;qty.form.submit();}">+</button>
                            </div>
                            {{-- Hapus tombol Update --}}
                        </div>
                    </form>
